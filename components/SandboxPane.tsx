...
function segmentsFromMatchResults(
  text: string,
    ...
    if (emptyMessage) {
      return (
        <div className="mt-2 rounded-lg border border-slate-700 bg-slate-900/40 px-3 py-2">
          <div className="text-xs font-medium text-green-400/90">
            {emptyMessage}
          </div>
        </div>
      );
...
export default function SandboxPane() {
  ...
  const [deniedMatchAll, setDeniedMatchAll] = useState<MatchAllResult | null>(
    null,
  );
    ...
    <div className="flex flex-col h-full border-t border-slate-700">
      <div className="flex-1 flex flex-col p-4 space-y-4 overflow-y-auto">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {pasteStatus === "ok" && (
            <span className="text-xs text-green-400">Pasted</span>
          )}
          {pasteStatus === "err" && (
            <span className="text-xs text-red-400">Paste failed</span>
          )}
          <input
            ref={fileInputRef}
            type="file"
            ...
            <ul className="space-y-1 max-h-24 overflow-y-auto">
              ...
              {batchTestStrings.length > 50 && (
                <li className="text-xs text-slate-500 px-2">
                  … and {batchTestStrings.length - 50} more
                </li>
              )}
            </ul>
          </div>
          ...
          <div className="rounded-lg border border-violet-500/50 bg-slate-900/80 p-3 space-y-3">
            <div className="flex items-center justify-between gap-2 flex-wrap">
              ...
              <div className="flex items-center gap-1">
                ...
                <span className="text-xs text-slate-400 min-w-16 text-center">
                  {debugSteps.length > 0
                    ? `${debugStepIndex + 1} / ${debugSteps.length}`
                    : "0 / 0"}
                </span>
                <button
                  type="button"
                  ...
                  disabled={
                    debugSteps.length === 0 ||
                    debugStepIndex >= debugSteps.length - 1
                  }
                  className="text-xs px-2 py-1 rounded border border-slate-600 text-slate-300 hover:bg-slate-800 disabled:opacity-40 disabled:cursor-not-allowed"
                >
                  Next →
            ...
            {debugSteps.length === 0 ? (
              <p className="text-xs text-slate-400">
                No match — enter a string that matches the regex to step
                through.
              </p>
            ) : (
              <div className="rounded border border-slate-700 bg-slate-950/80 px-3 py-2">
                <div className="text-xs text-slate-400 mb-1">
                  String position (cursor at step)
                </div>
                <div className="font-mono text-sm text-slate-200 break-all leading-relaxed flex flex-wrap items-center">
                  {Array.from(debugTestString).map((char, i) => (
                    <span
                      key={i}
                      className={
                        i === (debugSteps[debugStepIndex]?.stringIndex ?? 0)
                          ? "bg-violet-500/40 text-violet-200 rounded px-0.5"
                          : ""
                      }
                    >
                      {char === " " ? "\u00A0" : char}
                  ...
                  {(debugSteps[debugStepIndex]?.stringIndex ?? 0) >=
                    debugTestString.length && (
                    <span className="bg-violet-500/40 text-violet-200 rounded px-0.5 ml-0.5">
                      │
                    </span>
                  )}
                </div>
