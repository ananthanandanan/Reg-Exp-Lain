 ...
 */
function nodeToExplanation(node: AstNode<Features>, nodeId: string): ExplanationItem {
    let text = '';
    ...
    switch (node.type) {
        ...
        case 'group':
            if (node.behavior === 'normal') {
                ...
                text = 'Negative lookahead: matches only if NOT followed by the pattern';
                ...
                text = 'Negative lookbehind: matches only if NOT preceded by the pattern';
            } else {
...
function extractExplanationsRecursive(
    ...
    if (node.type === 'alternative') {
        ...
        node.body.forEach((child, index) => {
            extractExplanationsRecursive(child, `${nodeId}-child-${index}`, explanations);
        });
    } else if (node.type === 'disjunction') {
        node.body.forEach((child, index) => {
            extractExplanationsRecursive(child, `${nodeId}-branch-${index}`, explanations);
        });
    }
}
 ...
 */
export function astToExplanation(ast: RootNode<Features> | null): ExplanationItem[] {
    if (!ast) {
        return [];
    }
